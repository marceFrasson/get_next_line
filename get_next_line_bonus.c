...
/*   Updated: 2021/03/03 14:32:05 by mfrasson         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char		*statik[OPEN_MAX];
	char			*buffer;
	int				n;

	n = 1;
	if (line == NULL || BUFFER_SIZE <= 0 || fd < 0 || fd > RLIMIT_NOFILE)
		return (-1);
	if (!(buffer = (char*)malloc((BUFFER_SIZE + 1) * sizeof(char))))
		return (-1);
	if (!(loop(fd, buffer, &statik[fd], &n)))
		return (-1);
	statik[fd] = add_line(statik[fd], line);
	if (!statik[fd] && n == 0)
		return (0);
	return (1);
}
